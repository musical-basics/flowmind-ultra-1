...
export function NodeIndicator({ label, status, detail, isLast }: { label: string, status: NodeStatus, detail?: string, isLast?: boolean }) {
  ...
  const getColors = () => {
    switch(status) {
      ...
      case 'Healing': return { bg: 'bg-amber-500', border: 'border-amber-500', shadow: 'shadow-[0_0_15px_#f59e0b]', text: 'text-amber-500' };
      case 'AwaitingHumanFix': return { bg: 'bg-orange-600', border: 'border-orange-600', shadow: 'shadow-[0_0_15px_#ea580c]', text: 'text-orange-600' };
    ...
    <div className="flex items-center">
      <div className="flex flex-col items-center">
        <motion.div 
          initial={false}
          animate={{
            scale: (status === 'Active' || status === 'Healing') ? [1, 1.2, 1] : 1,
            opacity: status === 'Idle' ? 0.3 : 1
          }}
          transition={{ repeat: (status === 'Active' || status === 'Healing') ? Infinity : 0, duration: 1.5 }}
          className={`relative flex items-center justify-center w-5 h-5 rounded-full ${colors.bg} ${colors.shadow} mb-2 border-2 ${colors.border}`}
        >
          {status === 'Healing' && <span className="text-[10px] animate-spin absolute">🔧</span>}
        </motion.div>
